...
def test_color_palette_quantization():
    ...
    test_palette = load_color_palette()
    mario = Image.open("./sprites/MarioSmallFrame1.png")
    link = Image.open("./sprites/link_sprite.png")
    mario_q = mario.convert("RGB").quantize(colors=256, palette=test_palette)
    link_q = link.convert("RGB").quantize(colors=256, palette=test_palette)
...
def test_two_images_same_palette():
    ...
    samus1_q = samus1.convert("RGB").quantize(colors=256, palette=test_palette)
    samus2_q = samus2.convert("RGB").quantize(colors=256, palette=test_palette)
